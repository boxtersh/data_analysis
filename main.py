...
print(f'Для матрицы:\n'
      ...
      f'- 4. Датчики с наибольшей суммарной величиной:', *sensor_type,'\n',
      '''
5. Судя по предоставленной информации я вам ничего не порекомендую, так как:
- Датчики абсолютно разных типов что говорит о разных интервалах измеряемых значений, особенно 
для датчиков - PressureSensor и VibrationSensor.Для аналитики требуются датчики одного типа.
- Входные данные абсолютно разные, для справедливой оценки требуются схожие данные или хотя бы для разных датчиков 
подаются номинальные для них значения. Какие здесь представлены не понятно.
- Если учесть что каждый из датчиков получает тестовые, номинальные значения, тогда датчик VibrationSensor показывает 
дисперсию близкую к нулю, что является очень хорошим показателем, другие три датчика показываю умеренную дисперсию, 
что тоже является неплохим показателем.
Однако как было сказано ранее нам не известны характеристики: степени точности, погрешностей и диапазонов измерений
данных датчиков. Если принять что все датчики имеют одинаковые характеристики, но разные интервалы измеряемых значений, тогда 
датчик VibrationSensor является более точным в измерении и предпочтительнее.
      ''')